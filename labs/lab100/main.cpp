...
bool has_baseline_shape(const GameState& state) {
    return state.board.size() == BOARD_HEIGHT
        && !state.board.empty()
...
void print_state_summary(const GameState& state) {
    ...
    std::cout << "board=" << state.board.size() << "x" << state.board.front().size() << '\n';
    std::cout << "block=" << state.block_x << "," << state.block_y << '\n';
    std::cout << "shape=" << state.current_block.size() << "x" << state.current_block.front().size() << '\n';
