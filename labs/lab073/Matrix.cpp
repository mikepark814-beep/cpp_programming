...
Matrix::Matrix(std::initializer_list<std::initializer_list<int>> init)
    : rows(init.size()), cols(init.begin() -> size()), data(new int[init.size() * init.begin() -> size()]) {
...
Matrix::Matrix(const Matrix& other)
    : data(new int[other.rows * other.cols]), rows(other.rows), cols(other.cols) {
...
bool Matrix::operator==(const Matrix& rhs) const { return rows == rhs.rows && cols == rhs.cols && rg::equal(as_span(), rhs.as_span()); }
...
//       os << "\n";
//   }
std::ostream& operator<<(std::ostream& os, const Matrix& m) { for (auto i : rv::iota(0uz, m.rows)) {
    for (auto [j, val] : m.get_row(i) | rv::enumerate) {
        if (j > 0) os << " ";
        os << val;
    }
